...
bleprph_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            ...
            MODLOG_DFLT(INFO, "Passkey on device's display: %" PRIu32,
                        event->passkey.params.numcmp);
            MODLOG_DFLT(INFO, "Auto-aceptando emparejamiento (Sensor Autónomo)...");

            // BORRAMOS EL if (scli_receive_key...) Y PONEMOS ESTO:
            ...
            */
            ...
            ESP_LOGI(tag, "El movil pide un PIN. Usando PIN por defecto: 123456");
            pkey.action = event->passkey.params.action;

            // Ponemos el PIN fijo sin esperar al teclado. Error Ciberseguridad :(
            pkey.passkey = 123456;
